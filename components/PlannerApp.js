...
Data yang sekarang akan tertimpa dan tidak bisa dikembalikan.`)) return;
  ...
  return (
    <div className="planner-shell">
      <a className="lewati-ke-isi" href="#isi-planner">Lewati ke isi halaman</a>
      <aside className={`planner-sidebar ${sidebar ? "open" : ""}`}>
        ...
        <nav className="planner-nav" id="planner-nav" aria-label="Menu modul planner">
      ...
      <main className="planner-main">
        <header className="planner-topbar"><button className="mobile-menu" onClick={() => setSidebar(true)} aria-label="Buka menu navigasi" aria-expanded={sidebar} aria-controls="planner-nav">☰</button><div><span>WEDDING WORKSPACE</span><strong>{NAV.find(([id]) => id === view)?.[2]}</strong></div><div className="top-actions"><span className="save-state">● Tersimpan otomatis</span><button className="ghost-button" onClick={exportData} title="Unduh cadangan seluruh data">↓ Cadangkan</button><button className="avatar-button" aria-label={`Profil ${couple}`}>{couple.charAt(0)}</button></div></header>
        {storageError && <div className="storage-warning" role="alert"><strong>⚠ Data belum tersimpan.</strong> {storageError}</div>}
        <div className="planner-content" id="isi-planner" tabIndex={-1}>
...
function EmergencyKitView({ data, patchData, notify }) {
  ...
  return <><PageIntro kicker="SIAP SEDIA HARI-H" title="Emergency Kit Hari H" text="Checklist barang darurat yang wajib ada di lokasi — dari jarum jahit sampai powerbank — supaya masalah kecil tak jadi drama." action={<button className="main-button" onClick={() => setModal(true)}>＋ Tambah item</button>} />
    ...
    {kit.length === 0 ? <section className="workspace-card"><Empty text="Belum ada emergency kit. Muat daftar umum lalu sesuaikan dengan kebutuhanmu." /><div style={{ textAlign: "center" }}><button className="main-button" onClick={loadReference}>Muat daftar emergency kit umum</button></div></section>
      : <section className="workspace-card document-list">{kit.map((x) => <article key={x.id} className={x.checked ? "is-done" : ""}><button className="check-button" onClick={() => toggle(x.id)} aria-pressed={!!x.checked} aria-label={`${x.checked ? "Batalkan tanda siap" : "Tandai siap"}: ${x.item}`}>{x.checked ? "✓" : ""}</button><div><strong>{x.item}</strong><small>{x.category}</small></div><button className="delete-x" onClick={() => remove(x.id)} aria-label={`Hapus ${x.item || x.shot || x.task || "item"}`}>×</button></article>)}</section>}
...
function ShotListView({ data, patchData, notify }) {
  ...
  return <><PageIntro kicker="DAFTAR MOMEN FOTO" title="Shot List Dokumentasi" text="Daftar momen wajib untuk fotografer & videografer — supaya tak ada momen penting yang terlewat. Bagikan ke tim dokumentasimu." action={<button className="main-button" onClick={() => setModal(true)}>＋ Tambah shot</button>} />
    ...
    {shots.length === 0 ? <section className="workspace-card"><Empty text="Belum ada shot list. Muat daftar umum lalu sesuaikan dengan momen yang kamu inginkan." /><div style={{ textAlign: "center" }}><button className="main-button" onClick={loadReference}>Muat daftar shot umum</button></div></section>
      : <section className="workspace-card document-list">{shots.map((x) => <article key={x.id} className={x.checked ? "is-done" : ""}><button className="check-button" onClick={() => toggle(x.id)} aria-pressed={!!x.checked} aria-label={`${x.checked ? "Batalkan tanda" : "Tandai"}: ${x.shot}`}>{x.checked ? "✓" : ""}</button><div><strong>{x.shot}</strong><small>{x.category}</small></div><button className="delete-x" onClick={() => remove(x.id)} aria-label={`Hapus ${x.item || x.shot || x.task || "item"}`}>×</button></article>)}</section>}
...
function PascaNikahView({ data, patchData, notify }) {
  ...
  return <><PageIntro kicker="TUNTAS SETELAH ACARA" title="Checklist Pasca-Nikah" text="Urusan administrasi & adat setelah hari-H — dari lapor banjar sampai akta & KTP baru. Tandai satu per satu supaya tuntas dengan tenang." action={<button className="main-button" onClick={() => setModal(true)}>＋ Tambah tugas</button>} />
    ...
    {tasks.length === 0 ? <section className="workspace-card"><Empty text="Belum ada checklist. Muat daftar umum (lapor banjar, akta, KTP, BPJS) lalu sesuaikan." /><div style={{ textAlign: "center" }}><button className="main-button" onClick={loadReference}>Muat checklist pasca-nikah umum</button></div></section>
      : <section className="workspace-card document-list">{tasks.map((x) => <article key={x.id} className={x.checked ? "is-done" : ""}><button className="check-button" onClick={() => toggle(x.id)} aria-pressed={!!x.checked} aria-label={`${x.checked ? "Batalkan tanda selesai" : "Tandai selesai"}: ${x.task}`}>{x.checked ? "✓" : ""}</button><div><strong>{x.task}</strong><small>{x.when || "Tanpa tenggat"}</small></div><button className="delete-x" onClick={() => remove(x.id)} aria-label={`Hapus ${x.item || x.shot || x.task || "item"}`}>×</button></article>)}</section>}
...
function PendanaanView({ data, patchData, notify }) {
  ...
  return <><PageIntro kicker="SUMBER DANA & PATUNGAN" title="Pendanaan & Patungan" text="Siapa menyumbang berapa, dan berapa yang sudah benar-benar terkumpul. Hanya dana yang sudah diterima yang bisa dipakai membayar di muka." action={<button className="main-button" onClick={() => setModal(true)}>＋ Tambah sumber</button>} />
    ...
    {sources.length === 0 ? <section className="workspace-card"><Empty text="Belum ada sumber dana. Tambahkan kontributor (mis. Orang tua pria, tabungan pasangan) beserta jumlah yang dijanjikan." /></section>
      : <section className="workspace-card budget-table-card"><CardHead kicker="KONTRIBUSI" title="Sumber Dana" action={money(received)} /><div className="data-table budget-table"><div className="table-head"><span>Kontributor</span><span>Dijanjikan</span><span>Terkumpul</span><span>Sisa</span><span>Progres</span></div>{sources.map((s) => { const rec = Number(s.received || 0); const ratio = s.amount ? rec / s.amount * 100 : rec ? 100 : 0; return <div className="table-row" key={s.id}><strong>{s.name}<button className="delete-x inline" onClick={() => remove(s.id)} aria-label={`Hapus sumber dana ${s.name}`}>×</button></strong><label className="money-cell">Rp <input type="number" value={s.amount} onChange={(e) => update(s.id, "amount", e.target.value)} /></label><label className="money-cell">Rp <input type="number" value={rec} onChange={(e) => update(s.id, "received", e.target.value)} /></label><span className={s.amount - rec > 0 ? "" : "positive-text"}>{money(s.amount - rec)}</span><div className="row-progress"><i style={{ width: `${Math.min(ratio, 100)}%` }} /><small>{Math.round(ratio)}%</small></div></div>; })}</div></section>}
...
function ChecklistView({data,patchData,notify}) {
  ...
  return <><PageIntro kicker={`${data.checklist.length} TUGAS PERSIAPAN`} title="Checklist persiapan" text="Tentukan status, vendor, tenggat, dan catatan untuk setiap pekerjaan." action={<button className="main-button" onClick={()=>open()}>＋ Tambah tugas</button>} />
    ...
    <section className="workspace-card checklist-list">{items.map(item=><article key={item.id} className={item.done?"is-done":""}><button className="check-button" onClick={()=>toggle(item.id)} aria-pressed={!!item.done} aria-label={item.done?`Batalkan tanda selesai: ${item.task}`:`Tandai selesai: ${item.task}`}>{item.done?"✓":""}</button><div><strong>{item.task}</strong><small>{item.vendor||"Vendor belum dipilih"}{item.notes?` · ${item.notes}`:""}</small></div><select value={item.status} onChange={e=>patchData("checklist",data.checklist.map(x=>x.id===item.id?{...x,status:e.target.value,done:e.target.value==="DONE"}:x))}><option>BELUM</option><option>ON PROCESS</option><option>DONE</option></select><time dateTime={item.dueDate?.slice(0,10)||undefined} className={item.dueDate&&new Date(item.dueDate)<new Date()&&!item.done?"late":""}>{item.dueDate?prettyDate(item.dueDate):"Tanpa tenggat"}</time><div className="row-actions"><button onClick={()=>open(item)}>Edit</button><button onClick={()=>remove(item.id)}>Hapus</button></div></article>)}</section>
...
function PreweddingView({data,patchData,notify}) {
  ...
  return <><PageIntro kicker={`${data.preweddingLocations.length} REFERENSI LOKASI`} title="Perencanaan prewedding" text="Bandingkan biaya lokasi dan simpan tempat favorit untuk dibahas bersama."/><div className="prewed-feature">{photos.map(asset=><div className="prewed-foto" key={asset.src}><Image src={asset.src} alt="Referensi lokasi prewedding" fill sizes="(max-width: 860px) 33vw, 25vw" style={{objectFit:"cover"}} /></div>)}<div className="prewed-teks"><span>SHORTLIST</span><strong>{data.preweddingLocations.filter(x=>x.shortlisted).length} lokasi dipilih</strong><p>Gunakan ikon hati untuk menyimpan kandidat lokasi.</p></div></div><div className="list-toolbar"><input placeholder="Cari lokasi..." value={q} onChange={e=>setQ(e.target.value)}/><select value={sort} onChange={e=>setSort(e.target.value)}><option value="name">Urutkan nama</option><option value="price">Harga termurah</option></select></div><div className="location-grid">{locations.map(location=><article key={location.id} className={location.shortlisted?"selected":""}><button onClick={()=>toggle(location.id)} aria-pressed={!!location.shortlisted} aria-label={location.shortlisted?`Hapus ${location.name} dari pilihan`:`Simpan ${location.name} ke pilihan`}>{location.shortlisted?"♥":"♡"}</button><span>LOKASI</span><strong>{location.name}</strong><b>{money(location.price)}</b><small>{location.note||"Tidak ada catatan tambahan"}</small></article>)}</div></>;
}
...
function MoodboardView({data}) {
  ...
  return <><PageIntro kicker="REFERENSI VISUAL EXCEL" title="Mood board" text="Aset gambar dan seluruh catatan konsep dari workbook tersimpan di sini."/><div className="mood-layout"><section className="mood-gallery">{moodAssets.map((asset,index)=><figure key={asset.src} className={`mood-${index%5}`}><Image src={asset.src} alt={`Referensi ${asset.sheet}`} width={asset.width||800} height={asset.height||600} sizes="(max-width: 580px) 92vw, (max-width: 1180px) 46vw, 30vw" style={{width:"100%",height:"auto"}} /><figcaption>{asset.sheet} · {asset.column}{asset.row}</figcaption></figure>)}</section><aside className="workspace-card mood-notes"><CardHead kicker={`${sections.length} CATATAN`} title="Arahan Konsep"/>{sections.map((note,i)=><div key={`${note.cell}-${i}`}><span>{note.cell}</span><p>{note.text}</p></div>)}</aside></div></>;
}
...
function Modal({title,onClose,children}) { return <div className="planner-modal-bg" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="planner-modal"><header><div><span>DEWASA AYU</span><h2>{title}</h2></div><button onClick={onClose} aria-label="Tutup jendela">×</button></header>{children}</section></div>; }
